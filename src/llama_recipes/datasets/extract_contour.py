...
class MelodyContourExtractor:
    ...
    def extract_main_melody_track(self):
        ...
        for inst in melody_sequence:
            # Debug info! inst: [Note(onset=50, duration=50, abs_pitch=57), 
    ...
    def print_final_contour(self):
        ...
        for i, (start_time, iv, dur) in enumerate(final_contour):
            print(f"{i:2d}: start_time: {start_time}, Interval: {iv:+}, Duration: {dur}")
        # start_times = [start_timve for start_time, _ in final_contour]
        # contour_intervals = [iv for _, iv in final_contour]
        # print("\nCopy-paste ready:")
        # print("start_times =", start_times)
        # print("contour_intervals =", contour_intervals)

    def plot_all(self, contour=None):
        if contour is None:
        ...
        abs_path_x = contour_start_times + [smoothed_times[-1]]
...
if __name__ == "__main__":
    ...
    for idx in range(1, len(raw_melody_sequences) + 1):
        ...
        extractor.plot_all()
        plt.savefig(f'./zzz-figs/rdp_v2.1_{idx}.png', dpi=300, bbox_inches='tight')
        plt.close()
