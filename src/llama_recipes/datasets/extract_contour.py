...
class MelodyContourExtractor:
    ...
    def extract_main_melody_track(self):
        ...
        for inst in melody_sequence:
            ...
            if inst.name == -84:  # -84 timpani (drum)
                continue
        ...
        # ---- Step 2: resolve polyphony using pitch priority ----  TODO: to be improved
    ...
    def plot_with_duration_axis(self, contour=None):
        ...
        beat_times = [n.onset for n in notes]
        original_pitches = [n.abs_pitch for n in notes]
        
        # Extract data from contour
        contour_start_times = [start_time for start_time, _ in contour]
        contour_intervals = [interval for _, interval in contour]
        
        # Calculate absolute path (cumulative intervals)
        abs_path = [0]
        for interval in contour_intervals:
            abs_path.append(abs_path[-1] + interval)
        
        # Create x-coordinates for abs_path: use contour_start_times for all but the last one
        # For the last value, use the last beat_time from original_pitches
        abs_path_x = contour_start_times + [beat_times[-1]]
        
        plt.figure(figsize=(14, 6))
        
        plt.plot(beat_times, original_pitches, label="Original MIDI Pitch", marker='o', alpha=0.7)
        plt.scatter(contour_start_times, contour_intervals, label="Final Contour Intervals", marker='x', s=100, linewidth=2)
